...
├── tests\
│   └── Invoke-SafeRecoveryTests.ps1 # 恢复模块纯函数测试与红线静态自检 (不接触真实网络)
├── manifests\
...
5. **若当前网络本来就是可用的（无论在 `701` 还是 `702`），直接结束、不做任何修改**，
   并提示「无需恢复」。只有你显式使用 `-TargetProfile` 时才会主动切换（届时会红字警告将断开当前网络）。
6. 打印恢复前状态与恢复计划，然后要求你**逐字输入 `YES`**；直接回车即取消，不修改网络。
7. 依次尝试 `701` → `702`（每个配置文件仅在本机**已保存**时才尝试）：
   - `netsh wlan connect name=<配置文件> interface=<无线网卡>`，并检查退出码；
   - 轮询确认真正关联成功（退出码为 0 不代表已连上）；关联状态同时参考
     **WLAN 服务**与 **NLA**，任一确认即算关联，避免把「其实已连上」误判为失败；
   - 先等 DHCP 自动完成，仍拿不到地址时**只对该无线网卡**执行一次 `ipconfig /renew`（并检查退出码）；
   - 校验后置条件：配置文件正确、介质已连接、IPv4 非 `169.254.*`、该接口存在默认路由。
8. `701` 失败则自动回退 `702`；两者都失败即**停止**，并**尽力重新连回你操作前那个配置文件**
   （只做一次白名单内的连接，不做任何其他动作），然后写日志退出，不执行任何其他修复手段。
...
- `-WhatIf`：只打印计划，不执行任何网络修改（**不会请求 UAC**）

**退出码**：
- `0` = 成功（已恢复 / 本来就在线 / `-WhatIf` 预演），
- `1` = 环境或执行错误，**且未做任何修改**（未提权、网卡歧义、白名单配置文件均未保存等），
- `2` = 已经尝试过但网络仍不可用，
- `3` = 用户取消。

一旦发生过任何网络修改，失败一律报 `2`，绝不会伪装成 `1`。

**安全测试**：`tests\Invoke-SafeRecoveryTests.ps1` 是不接触真实网络的纯函数测试与红线静态自检，
可随时运行自查（`powershell.exe -NoProfile -ExecutionPolicy Bypass -File tests\Invoke-SafeRecoveryTests.ps1`）。
