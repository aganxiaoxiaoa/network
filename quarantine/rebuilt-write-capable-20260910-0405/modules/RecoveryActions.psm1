...
# Windows 10/11 便携网络诊断与恢复工具箱 - 安全恢复与状态变更模块
# 严格遵循：
...
function Save-NetworkSnapshot {
    ...
    $snapshotData = [PSCustomObject]@{
        ...
        WinInetProxy = [PSCustomObject]@{
            ProxyEnable   = if ($props -and $props.PSObject.Properties['ProxyEnable'] -and ($null -ne $props.ProxyEnable)) { [int]$props.ProxyEnable } else { $null }
            ProxyServer   = if ($props -and $props.PSObject.Properties['ProxyServer'] -and ($null -ne $props.ProxyServer)) { [string]$props.ProxyServer } else { $null }
            ProxyOverride = if ($props -and $props.PSObject.Properties['ProxyOverride'] -and ($null -ne $props.ProxyOverride)) { [string]$props.ProxyOverride } else { $null }
            AutoConfigURL = if ($props -and $props.PSObject.Properties['AutoConfigURL'] -and ($null -ne $props.AutoConfigURL)) { [string]$props.AutoConfigURL } else { $null }
            AutoDetect    = if ($props -and $props.PSObject.Properties['AutoDetect'] -and ($null -ne $props.AutoDetect)) { [int]$props.AutoDetect } else { $null }
        }
    }
...
function Invoke-SafeRepair {
    ...
    if (-not $PSCmdlet.ShouldProcess("本机网络环境", "执行安全修复 (刷新 DNS 与按条件续租 DHCP)")) {
        return
    }
    ...
    if ($nic.Found) {
        ...
        if ($ipIntf -and $ipIntf.Dhcp -eq 'Enabled') {
            ...
            Write-Host "   - [注意] 绝不执行 /release，避免在路由器响应慢时主动丢弃可用地址。" -ForegroundColor Gray
            try {
...
function Invoke-ReconnectAdapter {
    ...
    # ShouldProcess 先行拦截 WhatIf
    if (-not $PSCmdlet.ShouldProcess("$($nic.Name) (Index: $InterfaceIndex)", "禁用并重新启用物理网络适配器")) {
        return $false
    }
    ...
    # 3. 严格使用管道调用，并在 finally 中尽最大努力确保网卡重新启用
    # 严格避免不存在的 -InterfaceIndex 参数
...
function Invoke-DeepReset {
    ...
    if (@($staticInterfaces).Count -gt 0) {
        ...
        Write-Host "默认保护策略已拦截此操作。如确需重置，请先人工备份静态 IP 信息。" -ForegroundColor Red
        return $false
    }
    ...
    Write-Host " [提示] 本工具遵循安全规范，绝对不会自动重启电脑。" -ForegroundColor Yellow
...
function Invoke-ResetProxyToDirect {
    ...
    # ShouldProcess 先行拦截 WhatIf
    if (-not $PSCmdlet.ShouldProcess($regPath, "备份并清除当前用户 WinINET 代理设置，恢复直连")) {
        return $false
    }
...
function Restore-ProxyFromSnapshot {
    ...
    # 2. 若指针缺失或不可用，自动扫描 Reason=ResetProxy 的最新有效快照
