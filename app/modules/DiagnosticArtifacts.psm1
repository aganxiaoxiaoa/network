# ==============================================================================
# DiagnosticArtifacts.psm1
# 便携网络只读诊断工具箱 - 诊断工件生成与驱动备份模块
# 严格遵循：
#  1. 绝不导出 Wi-Fi 明文密码 (严禁明文导出密码)
#  2. 绝不收集敏感凭据与无关文件
#  3. 驱动仅使用 pnputil /export-driver 导出至 backups 目录，不安装、不卸载、不删除
#  4. 支持 CmdletBinding(SupportsShouldProcess = $true)
# ==============================================================================
...
function Export-NetworkDrivers {
    ...
    $driversOutDir = Join-Path $ToolRoot "backups\network-drivers-$ts"

    if (-not $PSCmdlet.ShouldProcess($driversOutDir, "导出第三方网络硬件驱动并生成清单")) {
        return $false
    }
    ...
    Write-Host "`n   - 驱动清单已写入: backups\network-drivers-$ts\driver-inventory.csv" -ForegroundColor Cyan
