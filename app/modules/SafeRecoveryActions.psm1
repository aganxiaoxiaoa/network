...
function Test-SafeRecoveryWaitCondition {
    ...
    if ($Condition -eq 'Associated') {
        return ($State.MediaConnectionState -eq 'Connected' -and $State.ConnectedProfile -eq $ExpectedProfile)
    }
...
function Wait-SafeRecoveryState {
    ...
    while ($true) {
        $state = Get-SafeRecoveryInterfaceState -InterfaceGuid $InterfaceGuid -GatewayPingCount $GatewayPingCount
...
function Invoke-SafeRecoveryProfileAttempt {
    ...
    Write-SafeRecoveryLog -LogPath $LogPath -Message "执行: $($connect.Command) => ExitCode=$connectExitCode" -ForegroundColor Yellow
    ...
    if (-not $connect.Executed) {
        return [pscustomobject]@{
            Profile = $ProfileName; Succeeded = $false; ConnectExitCode = $null
            RenewPerformed = $false; RenewExitCode = $null
            Reasons = @('WhatIf 模式: 未执行任何修改'); FinalState = $state
        }
    }
    if ($connectExitCode -ne 0) {
        return [pscustomobject]@{
            Profile = $ProfileName; Succeeded = $false; ConnectExitCode = $connectExitCode
            RenewPerformed = $false; RenewExitCode = $null
            Reasons = @("netsh wlan connect 返回非零退出码 $connectExitCode"); FinalState = $state
        }
    }
    ...
    if (-not $associated.Satisfied) {
        return [pscustomobject]@{
            Profile = $ProfileName; Succeeded = $false; ConnectExitCode = $connectExitCode
            RenewPerformed = $false; RenewExitCode = $null
            Reasons = @("在 $ConnectTimeoutSeconds 秒内未确认关联到 $ProfileName"); FinalState = $state
        }
    }
    ...
    if (-not (Test-SafeRecoveryIPv4Usable -IPAddress $state.IPv4Address)) {
        if ($state.DhcpEnabled -ne $true) {
            return [pscustomobject]@{
                Profile = $ProfileName; Succeeded = $false; ConnectExitCode = $connectExitCode
                RenewPerformed = $false; RenewExitCode = $null
                Reasons = @('该接口未启用 DHCP，按安全边界拒绝续租'); FinalState = $state
            }
        }
        ...
        $leased = Wait-SafeRecoveryState -InterfaceGuid $Adapter.InterfaceGuid -Condition 'UsableAddress' `
            -TimeoutSeconds $DhcpWaitSeconds -PollIntervalMilliseconds $pollInterval -GatewayPingCount $pingCount
        $state = $leased.State
    }
...
# ------------------------------------------------------------------------------
# 恢复编排 (唯一对外入口)
# 退出码: 0 成功 / 1 环境或执行错误 / 2 全部尝试后网络仍不可用 / 3 用户取消
# ------------------------------------------------------------------------------
function Invoke-SafeNetworkRecovery {
    ...
    if (-not (Test-SafeRecoveryAdmin)) {
        Write-SafeRecoveryLog -LogPath $logPath -Message '[中止] DHCP 续租需要管理员权限，当前不是管理员。' -ForegroundColor Red
        return [pscustomobject]@{ ExitCode = 1; Succeeded = $false; ProfileUsed = $null; LogPath = $logPath; Attempts = @() }
    }
    ...
    try {
        ...
        Write-SafeRecoveryLog -LogPath $logPath -Message "[中止] $($_.Exception.Message)" -ForegroundColor Red
        return [pscustomobject]@{ ExitCode = 1; Succeeded = $false; ProfileUsed = $null; LogPath = $logPath; Attempts = @() }
    }
    ...
    if ($plan.Count -eq 0) {
        Write-SafeRecoveryLog -LogPath $logPath -Message '[中止] 白名单内没有任何已保存的 Wi-Fi 配置文件可用。' -ForegroundColor Red
        return [pscustomobject]@{ ExitCode = 1; Succeeded = $false; ProfileUsed = $null; LogPath = $logPath; Attempts = @() }
    }
    ...
    if ($WhatIfPreference) {
        ...
        return [pscustomobject]@{ ExitCode = 0; Succeeded = $false; ProfileUsed = $null; LogPath = $logPath; Attempts = @() }
    }
    ...
    if ($answer.Trim() -cne $token) {
        ...
        return [pscustomobject]@{ ExitCode = 3; Succeeded = $false; ProfileUsed = $null; LogPath = $logPath; Attempts = @() }
    }
    ...
    if ($null -eq $succeededProfile) {
        ...
        Write-SafeRecoveryLog -LogPath $logPath -Message '        请手动在任务栏 Wi-Fi 中检查，或使用 START-DIAGNOSIS.cmd 采集证据后进一步排查路由器侧 DHCP。' -ForegroundColor Yellow
        return [pscustomobject]@{ ExitCode = 2; Succeeded = $false; ProfileUsed = $null; LogPath = $logPath; Attempts = $attempts.ToArray() }
    }
...
function Get-SafeRecoveryPreflight {
    ...
    $profileText = Format-SafeRecoveryProfileName -Name $state.ConnectedProfile -AllowedProfile $allowed
    Write-Host " 当前网络: $profileText | Media=$($state.MediaConnectionState) | IPv4=$($state.IPv4Address) | Dhcp=$($state.DhcpEnabled) | Gateway=$($state.DefaultGateway)" -ForegroundColor Gray
